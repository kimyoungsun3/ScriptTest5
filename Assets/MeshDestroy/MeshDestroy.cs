...
namespace MeshDestroyTest
{
	public class MeshDestroy : MonoBehaviour
	{
		...
		int cutCascades = 1;
		float explodeForce = 0;
		[SerializeField, Range(1, 20f)] float destroyTime = 10f;

		public LayerMask mask = 1 << 12;//(1 << 9) | (1 << 12) | (1 << 14);
		[SerializeField, Range(1f, 10f)] float perPiece = 1.5f;
		...
		private void OnCollisionEnter(Collision _col)
		{
			...
			int _value = (int)Mathf.Pow(2, _col.gameObject.layer);
			if ((_value & mask) > 0)
			{
				Set_Destroy();
			}
		...
		{
			Bounds _bounds	= GetComponent<MeshRenderer>().bounds;
			cutCascades		= (int)((_bounds.extents.magnitude + 1) * perPiece);
			cutCascades		= Mathf.Clamp(cutCascades, 1, 20);
			explodeForce	= 20f;
			DestroyMesh();
		}
		...
		private void DestroyMesh()
		{
			...
			var mainPart = new PartMesh(destroyTime)
			{
				UV			= originalMesh.uv,
			...
			for (var i = 0; i < parts.Count; i++)
			{
				...
				parts[i].go.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * explodeForce, transform.position);
			}
		...
		{
			var partMesh = new PartMesh(destroyTime) { };
			var ray1 = new Ray();
			var ray2 = new Ray();
		...
		public class PartMesh
		{
			...
			public PartMesh(float _destroyTime)
			{
			...
			public void MakeGameobject(MeshDestroy original)
			{
				...
				Destroy(collider, destroyTime - 1f);
