...
public class JSONObject {
	...
	public static JSONObject Create()
	{
		...
		//Debug.Log(" >>> JSONObject Create === start :" + releaseQueue.Count);
#if POOLING
		while (result == null && releaseQueue.Count > 0)
		{
			result = releaseQueue.Dequeue();
			Debug.Log(" >>> JSONObject Create === reuse " + result.serialNumber + ":" + releaseQueue.Count);
#if DEV
...
#if POOLING
		result.serialNumber = newCount++;		
		Debug.Log(" >>> JSONObject Create === new create  :" + newCount);
#endif
		return result;
	}
#if POOLING
...
#region STRINGIFY
	...
	public static void ClearPool() {
		...
		releaseQueue.Clear();
		pool = true;
		Debug.Log(" >>> JSONObject ClearPool");
	}
	...
	~JSONObject() {
		if(pool && releaseQueue.Count < MAX_POOL_SIZE) {
			...
			Debug.Log(" >>> JSONObject ~JSONObject : " + serialNumber + " remain:" + releaseQueue.Count);
		}
	}
#endif
}