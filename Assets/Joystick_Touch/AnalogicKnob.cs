...
namespace LeoLuz.PlugAndPlayJoystick
{
    public class AnalogicKnob : ButtonBase
    {
        ...
        public override void Start()
        {
            ...
            AnchoredAreaBounds = new Bounds(new Vector2(NormalizedAnchoredAreaBounds.center.x * _canvasRect.sizeDelta.x, NormalizedAnchoredAreaBounds.center.y * _canvasRect.sizeDelta.y)
                , new Vector2(NormalizedAnchoredAreaBounds.size.x * _canvasRect.sizeDelta.x, NormalizedAnchoredAreaBounds.size.y * _canvasRect.sizeDelta.y));
            ...
            CanvasScale = _canvas.transform.lossyScale.x;
            ...
            canvasInitialPoint = _canvas.transform.position + (new Vector3(-CanvasSize.x * _canvas.transform.lossyScale.x * 0.5f, -CanvasSize.y * _canvas.transform.lossyScale.y * 0.5f));
            Vector3 knobUnAnchoredPositionOnCanvas = (transHandle.position - canvasInitialPoint) / _canvas.transform.lossyScale.y;
            ...
            if (autoHide)
            {
                handleImage.CrossFadeAlpha(0f, 5f, true);
		...
		void TouchBegan(Vector2 TouchPosition)
        {
			...
			//
			Vector3 _hitPoint = canvasInitialPoint + (Vector3)TouchPosition * ScreenUnitsToWorldUnitsConversionConstant;
			KnobBackground.transform.position	= _hitPoint;
			transHandle.position				= _hitPoint;
			startPosition						= _hitPoint;


			if (autoHide)
            {
                handleImage.CrossFadeAlpha(1f, fadeInDuration, true);
                if (KnobBackground != null)
                {
                    KnobBackground.CrossFadeAlpha(1f, fadeInDuration, true);
                    if (knobBackgroundFollowKnob)
                    {
                        KnobBackground.transform.position	= transHandle.position;
                        startPosition				= transHandle.position;
                    }
                }
            }
        }
        ...
        void TouchMove(Vector3 position)
        {
			...
			transHandle.position = canvasInitialPoint + (Vector3)((StartPosition + (NormalizedAxis * _MaxDistance)) * ScreenUnitsToWorldUnitsConversionConstant);
        ...
        void TouchEnd()
        {
            ...
            if (autoHide)
            {
                handleImage.CrossFadeAlpha(0f, fadeOutDuration, true);
                if (KnobBackground != null)
                {
