...
public class ScrollBGTest : MonoBehaviour {
	...
	void Update () {
		...
		if (bScrolling) {
			...
			Debug.DrawLine (
				new Vector3((layers [leftIndex].transform.position.x + viewZone), 0, 0),
				new Vector3((layers [rightIndex].transform.position.x - viewZone), 0, 0),
				Color.green);
			Debug.Log( _posX + ", " + (layers [leftIndex].transform.position.x + viewZone) + ", " + (layers [rightIndex].transform.position.x - viewZone));
			if (_posX < (layers [leftIndex].transform.position.x + viewZone)) {
				ScrollLeft ();
			}

			//if (_posX > (layers [rightIndex].transform.position.x - viewZone)) {
			//	ScrollRight ();
			//}
		}
	}
